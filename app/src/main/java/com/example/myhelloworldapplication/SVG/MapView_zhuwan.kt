...
class MapView1 : View {
    ...
    private fun init(context: Context, attrs: AttributeSet) {
        ...
        //子线程解析 xml
        Thread {
            val inputStream = context.resources.openRawResource(R.raw.map1high)
    ...
    private fun drawMap(canvas: Canvas) {
        ...
        canvas.restore()
        canvas.drawText("Cheoc-Van Beach", widthSize / 2 - mPaintTextTitle.measureText("Cheoc-Van Beach") / 2f, 100f, mPaintTextTitle)
    }
    ...
    private fun handlerTouch(x: Float, y: Float) {
        ...
        mapDataList.forEach { data ->
            run {
                data.isSelect = false
                if (isTouchRegion1(x / xScale, y / yScale, data.pathData)) {
                    data.isSelect = true
                    postInvalidate()
                }
            }
 ...
 * 判断是否在点击区域内
 */
fun isTouchRegion1(x: Float, y: Float, path: Path): Boolean {
    //创建一个矩形
    val rectF = RectF()
    //获取到当前省份的矩形边界
