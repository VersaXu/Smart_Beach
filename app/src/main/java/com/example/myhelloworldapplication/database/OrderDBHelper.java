package com.example.myhelloworldapplication.database;
...
public class OrderDBHelper extends SQLiteOpenHelper {
    ...
    private static OrderDBHelper mHelper = null; // 数据库帮助器的实例
    ...
    private OrderDBHelper(Context context, int version) {
        super(context, DB_NAME, null, version);
    }

    // 利用单例模式获取数据库帮助器的唯一实例
    public static OrderDBHelper getInstance(Context context, int version) {
        if (version > 0 && mHelper == null) {
            mHelper = new OrderDBHelper(context, version);
